...
public class LegacyDataMigrationTests
{
    ...
    public async Task MovesLegacyDirectoryAndRewritesConfigs()
    {
        ...
        WriteFile(oldDirectory.Combine("Configs/Apocrypha.DataModel.DataModelSettings.json"), LegacyDataModelSettingsJson);
        WriteFile(oldDirectory.Combine("Configs/Apocrypha.Games.FileHashes.FileHashesServiceSettings.json"), HashesSettingsJson);
        ...
        var dataModelJson = await newDirectory.Combine("Configs/Apocrypha.DataModel.DataModelSettings.json").ReadAllTextAsync();
        ...
        var hashesJson = await newDirectory.Combine("Configs/Apocrypha.Games.FileHashes.FileHashesServiceSettings.json").ReadAllTextAsync();
    ...
    {
        using var sandbox = Sandbox.Create();
        WriteFile(sandbox.LocalAppData.Combine(ApplicationIdentity.LegacyDataDirectoryName).Combine("Configs/Apocrypha.DataModel.DataModelSettings.json"), LegacyDataModelSettingsJson);
    ...
    public async Task CrashBetweenMoveAndRewrite_SelfHealsOnNextRun()
    {
        ...
        var newDirectory = sandbox.LocalAppData.Combine(ApplicationIdentity.DataDirectoryName);
        WriteFile(newDirectory.Combine("Configs/Apocrypha.DataModel.DataModelSettings.json"), LegacyDataModelSettingsJson);

        var didWork = LegacyDataMigration.Run(sandbox.FileSystem, _ => { });

        await Assert.That(didWork).IsTrue();
        var json = await newDirectory.Combine("Configs/Apocrypha.DataModel.DataModelSettings.json").ReadAllTextAsync();
