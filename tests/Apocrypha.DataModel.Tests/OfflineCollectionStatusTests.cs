...
public class OfflineCollectionStatusTests(ITestOutputHelper helper) : AArchivedDatabaseTest(helper)
{
    ...
    /// <summary>
    /// CHARACTERISATION of the defect behind review finding S5-1 — this pins today's behaviour, and
    /// is expected to change when S5-1 is fixed.
    ///
    /// <para>
    ...
    /// the group carries its <see cref="NexusCollectionItemLoadoutGroup"/> tag. The install branches
    /// commit the group first, then apply curator patches, then tag in a second transaction — so a
    /// patch failure in between leaves a group that is deployed, unpatched and untagged, yet still
    /// reports installed. The install job skips anything reporting installed, so no retry ever heals
    /// it.
    /// </para>
    /// </summary>
    [Fact]
