...
using Apocrypha.Abstractions.EpicGameStore.Values;
...
using Apocrypha.Games.FileHashes.DTOs;
using NexusMods.Hashing.xxHash3;
using NexusMods.Hashing.xxHash3.Paths;
using NexusMods.MnemonicDB;
using NexusMods.MnemonicDB.Abstractions;
using NexusMods.MnemonicDB.Storage;
using Apocrypha.Networking.EpicGameStore.DTOs.EgData;
...
using Build = Apocrypha.Networking.EpicGameStore.DTOs.EgData.Build;
using BuildId = Apocrypha.Abstractions.GOG.Values.BuildId;
using Manifest = Apocrypha.Abstractions.Steam.DTOs.Manifest;
using OperatingSystem = Apocrypha.Abstractions.Games.FileHashes.Values.OperatingSystem;

namespace Apocrypha.Games.FileHashes.VerbImpls;
...
public class BuildHashesDb : IAsyncDisposable
{
    ...
    private readonly NexusMods.MnemonicDB.Storage.RocksDbBackend.Backend _backend;
    ...
    public BuildHashesDb(IRenderer renderer, IServiceProvider provider, TemporaryFileManager temporaryFileManager, IGameRegistry gameRegistry)
    {
        ...
        _backend = new NexusMods.MnemonicDB.Storage.RocksDbBackend.Backend();
    ...
    private async Task AddGogData(AbsolutePath path)
    {
        ...
        var builds = await LoadAllFromFolder<Apocrypha.Abstractions.GOG.DTOs.Build>(path / "json" / "stores" / "gog" / "builds");
    ...
    private async Task AddEpicGameStoreData(AbsolutePath path)
    {
        ...
        foreach (var (id, build) in metadata)
        {
            try
            {
                ...
                {
                    BuildId = Apocrypha.Abstractions.EpicGameStore.Values.BuildId.FromUnsanitized(build.Id),
